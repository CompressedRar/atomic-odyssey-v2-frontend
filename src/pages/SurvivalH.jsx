...
const SurvivalH = () => {
  ...
  const SAFE_VERTICAL_GAP = CARD_HEIGHT * 1.2; // 360px
  ...
  const saveScoreToLeaderboard = async (answeredQuestions) => {
    try {
      ...
      const leaderboardRef = ref(db, `leaderboards/hardSurvival/${user.uid}`);
  ...
  useEffect(() => {
    ...
    const update = () => {
      setFallingCards(prev => {
        ...
        if (survived.length < maxFalling && Math.random() < 0.08) {
  ...
  const fetchLeaderboard = () => {
    const db = getDatabase();
    const lbRef = ref(db, "leaderboards/hardSurvival");
  ...
  return (
    <div className="survival-container">
      ...
      {/* --- Centered Box Flashlight Effect (Smooth & Natural Pulse) --- */}
      <div
        style={{
          position: "fixed",
          inset: 0,
          background: `
      linear-gradient(to right, rgba(0,0,0,1) 0%, rgba(0,0,0,0.88) 25%, rgba(255,255,255,0) 50%, rgba(0,0,0,0.88) 75%, rgba(0,0,0,1) 100%),
      linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,0.88) 25%, rgba(255,255,255,0) 50%, rgba(0,0,0,0.88) 75%, rgba(0,0,0,1) 100%)
    `,
          backgroundBlendMode: "multiply",
          pointerEvents: "none",
          zIndex: 5,
          transition: "all 2s",
          animation: "flashlightPulse 5s infinite", // slower and smoother
        }}
      />
...
`}
      ...
      <div className="ui-container" style={{ position: "relative", zIndex: 10 }}>
        ...
        <h1>Periodic Table Survival - Hard</h1>
